...
/**
 * Command to set a faction's primary color.
 ...
 */
public class CmdFactionsColorSetPrimary extends FactionsCommand
{
	// -------------------------------------------- //
	// CONSTRUCT
	// -------------------------------------------- //
	
	public CmdFactionsColorSetPrimary()
	{
		// Aliases
		this.addAliases("p");
		
		// Parameters
		this.addParameter(TypeString.get(), "color").setDesc("the primary color (hex format #RRGGBB, or nothing to reset)");
	...
	{	
		...
		// Inform sender
		msg("<i>You set the primary color for <h>%s<i> to: <h>%s", faction.getName(msender), colorDisplay);
		
		// Inform other faction members
		for (MPlayer follower : faction.getMPlayers())
		{
			if (follower.equals(msender)) continue;
			follower.msg("<i>%s <i>set your faction primary color to: <h>%s", MixinDisplayName.get().getDisplayName(sender, follower), colorDisplay);
		}
	}
