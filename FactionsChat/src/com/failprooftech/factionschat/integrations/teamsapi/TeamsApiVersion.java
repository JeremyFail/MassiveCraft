...
/**
 * TeamsAPI semantic-version gate for FactionsChat consumer integration.
 * <p>
 * FactionsChat uses TeamsAPI-backed bridges and subcommands only when the runtime library reports
 * {@link #MINIMUM_API_VERSION} or newer (role prefix and {@link com.skyblockexp.teamsapi.model.TeamRoleDefinition}
 * support). Older API jars load without error, but TeamsAPI integration is skipped after a single console message
 * and callers fall back to direct Factions or FactionsBridge wiring.
 */
public final class TeamsApiVersion
{
	/** Minimum TeamsAPI {@link TeamsAPI#API_VERSION} required for TeamsAPI-backed chat data and subcommands. */
	 ...
	 * <p>
	 * Callers should treat {@code false} as "do not use TeamsAPI bridges or subcommands"; FactionsChat continues
	 * to load and should fall back to other faction integrations when available.
	 *
	 * @param logger FactionsChat plugin logger; may be {@code null} to suppress logging
