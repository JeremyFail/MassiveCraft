 ...
 * against {@link #MINIMUM_API_VERSION}. TeamsAPI bridges are skipped when the installed TeamsAPI plugin is older.
 */
public final class TeamsApiVersion
{
	...
	/** Minimum {@code TeamsAPI.API_VERSION} required for TeamsAPI-backed chat data. */
	 ...
	 * <p>
	 * A static reference to {@code TeamsAPI.API_VERSION} in FactionsChat source is not reliable: the compiler may
	 * inline the compile-time {@code provided} teams-api constant (e.g. 2.4.0), which can differ from the TeamsAPI
	 * plugin jar on the server.
	 *
	 ...
	 * <p>
	 * Used to load the constant from the TeamsAPI plugin jar rather than from FactionsChat's compile classpath.
	 *
