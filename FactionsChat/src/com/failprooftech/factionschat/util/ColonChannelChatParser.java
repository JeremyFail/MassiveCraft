...
public final class ColonChannelChatParser
{
    ...
    public static ParseResult parse(Player player, String plainText)
    {
        ...
        ChatMode mode = FactionsChatDispatcher.parseChatMode(token);
        if (mode == null)
        {
            return strictErrors
                ? invalidQuickChatGeneric()
    ...
    {
        return new ParseResult(ParseType.INVALID, null, null, ChatTxt.parse("<b>Invalid chat mode or command."));
    }
    ...
    private static String validateModeForPlayer(Player player, ChatMode mode)
    {
        ...
        // Check faction membership when the bridge is available
        ...
        {
            return ChatTxt.parse("<b>Cannot use that chat mode as you are not in a faction.");
        }
        return null;
    }
}
