...
public class CmdFactionsChatHelp extends FactionsCommand
{
    ...
    private List<String> buildHelpContent()
    {
        ...
        boolean isConsole = msender.isConsole();
        Player player = isConsole ? null : msender.getPlayer();
        
        // Chat Modes Section - only show if not console
        if (!isConsole)
        {
        ...
        if (isConsole ||
                ( player.hasPermission("factions.chat.ignore") 
                ...
                || player.hasPermission("factions.chat.reload")))
        {
            ...
            if (isConsole || player.hasPermission("factions.chat.ignore.admin"))
            {
                lines.add(Txt.parse("<i>  - <v>ignore [playerToUpdate] <player><i> - <i>Add players to the ignore list for " + (!isConsole ? "yourself or " : "") + "another player"));
                lines.add(Txt.parse("<i>  - <v>unignore [playerToUpdate] <player><i> - <i>Remove players from the ignore list for " + (!isConsole ? "yourself or " : "") + "another player"));
                lines.add(Txt.parse("<i>  - <v>ignorelist [player]<i> - <i>View the ignore list for " + (!isConsole ? "yourself or " : "") + "another player"));
            }
            else if (player.hasPermission("factions.chat.ignore"))
            {
                lines.add(Txt.parse("<i>  - <v>ignore <player><i> - <i>Add a player to " + (!isConsole ? "your" : "") + " ignore list"));
                lines.add(Txt.parse("<i>  - <v>unignore <player><i> - <i>Remove a player from " + (!isConsole ? "your" : "") + " ignore list"));
                lines.add(Txt.parse("<i>  - <v>ignorelist<i> - <i>View " + (!isConsole ? "your" : "") + " ignore list"));
            }
            
            if (isConsole || player.hasPermission("factions.chat.toggle.admin"))
            {
                lines.add(Txt.parse("<i>  - <v>toggle [player] <chatMode><i> - <i>Toggle (disable/enable) chat modes for " + (!isConsole ? "yourself or " : "") + "another player"));
            }
            ...
            if (isConsole || player.hasPermission("factions.chat.reload"))
            {
