...
public final class GsonMongoConverter
{
	...
	{
		key = StringUtils.replace(key, DOT_NORMAL, DOT_MONGO, -1);
		key = StringUtils.replace(key, DOLLAR_NORMAL, DOLLAR_MONGO, -1);
		return key;
	}
	...
	{
		key = StringUtils.replace(key, DOT_MONGO, DOT_NORMAL, -1);
		key = StringUtils.replace(key, DOLLAR_MONGO, DOLLAR_NORMAL, -1);
		return key;
	}
